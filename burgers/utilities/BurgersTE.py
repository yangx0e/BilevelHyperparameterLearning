...
class Burgers_GP(object):
    ...
    def eval_ops(self, kernel, u_weights, nx, params):
        ...
        xx0h = jnp.tile(jnp.transpose(x0), (nxl, 1)).flatten()
        xx1v = jnp.tile(nx1, self.M).flatten()
        xx1h = jnp.tile(jnp.transpose(x1), (nxl, 1)).flatten()
        ...
        # ut
        mtx = jnp.zeros((nxl, self.M + 3 * self.M_Omega))
        ...
        val = vmap(lambda x1, x2, y1, y2: kernel.D_x1_DD_y2_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, 2 * self.M_Omega:3 * self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))

        val = vmap(lambda x1, x2, y1, y2: kernel.D_x1_kappa(x1, x2, y1, y2, params))(xx0v, xx1v, xx0h, xx1h)
        mtx = mtx.at[:, 3 * self.M_Omega:].set(jnp.reshape(val, (nxl, self.M)))

        ut = mtx.dot(u_weights)
        ...
        val = vmap(lambda x1, x2, y1, y2: kernel.D_x2_DD_y2_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, 2 * self.M_Omega:3 * self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))

        val = vmap(lambda x1, x2, y1, y2: kernel.D_x2_kappa(x1, x2, y1, y2, params))(xx0v, xx1v, xx0h, xx1h)
        mtx = mtx.at[:, 3 * self.M_Omega:].set(jnp.reshape(val, (nxl, self.M)))

        ux = mtx.dot(u_weights)

        # uxx
        mtx = jnp.zeros((nxl, self.M + 3 * self.M_Omega))

        val = vmap(lambda x1, x2, y1, y2: kernel.DD_x2_D_y1_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, 0:self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))

        val = vmap(lambda x1, x2, y1, y2: kernel.DD_x2_D_y2_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, self.M_Omega:2 * self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))

        val = vmap(lambda x1, x2, y1, y2: kernel.DD_x2_DD_y2_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, 2 * self.M_Omega:3 * self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))

        val = vmap(lambda x1, x2, y1, y2: kernel.DD_x2_kappa(x1, x2, y1, y2, params))(xx0v, xx1v, xx0h, xx1h)
        mtx = mtx.at[:, 3 * self.M_Omega:].set(jnp.reshape(val, (nxl, self.M)))

        uxx = mtx.dot(u_weights)
        ...
        val = vmap(lambda x1, x2, y1, y2: kernel.DD_y2_kappa(x1, x2, y1, y2, params))(xxint0v, xxint1v, xxint0h, xxint1h)
        mtx = mtx.at[:, 2 * self.M_Omega:3 * self.M_Omega].set(jnp.reshape(val, (nxl, self.M_Omega)))
    ...
    def trainGN(self, init_params, cfg, params_reg_func = None):
        ...
        while  it < cfg.epoch: #error > cfg.tol and iter < cfg.epoch:
            ...
            batch_samples_interior_valid = jnp.array(batch_samples_interior_valid)
            ukt_vals, ukx_vals, ukxx_vals, uk_vals = self.eval_ops(
                u_kernel, uk_weights, batch_samples_interior_valid, uk_params,
            )
            zk_at_valid = jnp.concatenate((ukt_vals, ukx_vals, ukxx_vals, uk_vals))
            ...
            def loss_learning_params(params, zk_at_valid, batch_samples_interior_valid):
                ...
                # L = jnp.linalg.cholesky(gram_u)

                b = -jnp.dot(J.T, jsp.linalg.cho_solve(cho, y))
                # b = -jnp.dot(J.T, jnp.linalg.solve(L.T, jnp.linalg.solve(L, y)))
                tmp = jsp.linalg.cho_solve(cho, J)
                # tmp = jnp.linalg.solve(L.T, jnp.linalg.solve(L, J))
                ...
                # u_weights = jnp.linalg.solve(L.T, jnp.linalg.solve(L, new_uz))

                ut_vals, ux_vals, uxx_vals, u_vals = self.eval_ops(
                    u_kernel, u_weights, batch_samples_interior_valid, params,
                )
                z_at_valid = jnp.concatenate((ut_vals, ux_vals, uxx_vals, u_vals))
                # z_at_valid = jax.lax.stop_gradient(z_at_valid)
        ...
        path = pathlib.Path("./checkpoints/model_{}".format(str(it)))
