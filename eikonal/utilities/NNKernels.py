...
class LengthScaleNetwork(nn.Module):
...
class GibbsKernel1D:
    ...
    def kappa(self, x1, x2, y1, y2, params):
        ...
        out_x = self.output_fn(params, input_x).reshape([])
        out_y = self.output_fn(params, input_y).reshape([])

        k1 = jnp.exp(-(x1 - y1)**2 / (out_x**2 + out_y**2))
        k2 = jnp.exp(-(x2 - y2)**2 / (out_x**2 + out_y**2))
        k1 = k1 * jnp.sqrt(2*out_x*out_y / (out_x**2 + out_y**2))
        k2 = k2 * jnp.sqrt(2*out_x*out_y / (out_x**2 + out_y**2))
        k = k1 * k2
        return k
...
class KernelGenerator1D:
    ...
    def __call__(self):
        return GibbsKernel1D(self.ls_net)
