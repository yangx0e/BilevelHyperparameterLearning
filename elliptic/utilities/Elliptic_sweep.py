...
class Elliptic(object):
    ...
    def sampling(self, cfg):
        ...
        # Pre-generated collocation points for reproducibility
    ...
    def trainGN(self, init_params, cfg, VERBOSE_RETURN=False, my_grid=None):
        error = 1
        it = 0
        ...
        loss_history = []
        sweep_history = np.zeros((cfg.epoch, my_grid.shape[0]))
        np.save("./outputs/my_grid.npy", my_grid)
        while it < cfg.epoch:  # error > cfg.tol and it < cfg.epoch:
            ...
            ########### BEGIN SWEEP ###########
            for i, ell in enumerate(my_grid):
                ell = ell * jnp.ones(1)
                ell_loss = loss_learning_params(ell, zk_at_valid, batch_samples_interior_valid)
                sweep_history[it, i] = ell_loss
            
            np.save("./outputs/sweep_history_01.npy", sweep_history)
            ########### END SWEEP ###########
            ...
            plt.colorbar()
            plt.tight_layout()
            plt.savefig("./figures/u_pred_train.png")
            plt.close()
            ...
            plt.colorbar()
            plt.tight_layout()
            plt.savefig("./figures/u_ref_train.png")
            plt.close()
        ...
        np.savetxt("./outputs/params_history_01", params_history)
        np.savetxt("./outputs/loss_history_01", loss_history)
        np.save("./outputs/sweep_history_01.npy", sweep_history)
        u_func = self.gen_u(kernel_params, zk, zk, cfg)
        if VERBOSE_RETURN:
            u_func = (u_func, params_history, loss_history, sweep_history)
        return u_func
