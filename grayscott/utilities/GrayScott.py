...
class GrayScott(object):
    ...
    def sampling(self, cfg):
        ...
        # self.u0 = -jnp.sin(3*np.pi*x[self.M_Omega:self.M_Omega+self.M_0] + np.pi/2) * (t[self.M_Omega:self.M_Omega+self.M_0] == 0)
        # # self.u0 = -jnp.sin(7*np.pi*x[self.M_Omega:self.M_Omega+self.M_0] + np.pi/2) * (t[self.M_Omega:self.M_Omega+self.M_0] == 0)
        # self.v0 = np.cos(2*np.pi*x[self.M_Omega:self.M_Omega+self.M_0]) * (t[self.M_Omega:self.M_Omega+self.M_0] == 0)
        ...
        #         "M": cfg.M, "M_Omega": cfg.M_Omega, "M_0": self.M_0, "M_b": self.M_b,
        #         "u0": self.u0, "v0": self.v0, "dub": self.dub, "dvb": self.dvb,
        #         "samples_interior_valid": self.samples_interior_valid,
        #         "valid_M": cfg.valid_M, "valid_M_Omega": cfg.valid_M_Omega,
        #     }
        # )

        data = sio.loadmat("./data/grayscott.mat")
        ...
        self.M_b = data["M_b"].astype(np.int32).reshape([])
        self.u0 = data["u0"].flatten()
        self.v0 = data["v0"].flatten()
        self.dub = data["dub"].flatten()
        self.dvb = data["dvb"].flatten()
